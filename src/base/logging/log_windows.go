...
func (b *LogBackend) Log(level Level, calldepth int, strMsg string) error {
	outPut := fmt.Sprintf("%-8s %s", levelNames[level], strMsg)
	if b.Color && b.f != nil {
		setConsoleTextAttribute(b.f, colors[level])
		err := b.Logger.Output(calldepth+4, outPut)
		setConsoleTextAttribute(b.f, fgWhite)
		return err
	}
	return b.Logger.Output(calldepth+4, outPut)
}
